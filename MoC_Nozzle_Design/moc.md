...
This document provides an overview of the `moc.py` Python script, which implements the Method of Characteristics (MoC) for designing rocket nozzles.
The script calculates the nozzle contour and Mach number distribution for a given set of input parameters.

## Overview

The `moc.py` script is designed to compute the nozzle contour using the Method of Characteristics.
It includes functions to calculate Prandtl-Meyer functions, expansion ratios, and other necessary parameters for nozzle design.

## Key Functions

### Prandtl-Meyer Functions

- `prandtl_meyer(nu, gamma, M)`: Computes the Mach number for a given Prandtl-Meyer angle `nu`, specific heat ratio `gamma`, and initial guess `M`.
- `_nu_M(M, gamma)`: Computes the Prandtl-Meyer function for a given Mach number `M` and specific heat ratio `gamma`.
- `_dnu_dM(M, gamma)`: Computes the derivative of the Prandtl-Meyer function with respect to Mach number `M`.

### Area Ratio Functions

- `A_Astar(A_Astar, gamma, M)`: Computes the Mach number for a given area ratio `A/A*`, specific heat ratio `gamma`, and initial guess `M`.
- `_F_M(M, gamma, A_Astar)`: Computes the function for area ratio calculation.
- `_dF_dM(M, gamma)`: Computes the derivative of the area ratio function with respect to Mach number `M`.

### Nozzle Design Class

- `MethodOfCharacteristics`: A class that encapsulates the MoC nozzle design process.
    - `__init__(self, gamma=1.4)`: Initializes the class with a specific heat ratio `gamma`.
    - `_compat_constant(self, M, theta, left=True)`: Computes the compatibility constant for a given Mach number `M` and flow angle `theta`.
    - `_nozzle_properties(self, D_t, theta_e, M_e)`: Computes nozzle properties such as expansion ratio, nozzle length, and coordinates.
    - `_bell_coefficients(self, xp_e, yp_e, theta_n, theta_e)`: Computes coefficients for the bell nozzle contour.
    - `_point3_props(self, K1, K2)`: Computes properties at a specific point in the nozzle.
    - `_mach_angle(self, M)`: Computes the Mach angle for a given Mach number `M`.
    - `_char_slope(self, theta, mu)`: Computes the characteristic slopes for given flow angles `theta` and Mach angles `mu`.
    - `_point3_xy(self, m_cm, m_cp, x, y)`: Computes the coordinates of a specific point in the nozzle.
    - `_unit_internal(self, M, theta, xy1, xy2)`: Computes internal unit properties for the nozzle.
    - `_straightening_section(self, plot)`: Computes the straightening section of the nozzle.
    - `_unit_centerline(self, nu_w, theta_w, M_w, mu_w)`: Computes centerline properties for the nozzle.
    - `_wall_xy(self, m_cp, theta_s, x, y)`: Computes wall coordinates for the nozzle.
    - `_expansion_grid(self, plot)`: Computes the expansion grid for the nozzle.
    - `moc_nozzle_design(self, M_exit, D_t, gamma=1.4, N=10, bell=True, R_c=0., theta_e=None, plot=True)`: Main function to design the nozzle using MoC.
        - `M_exit`: Exit Mach number.
        - `D_t`: Throat diameter.
        - `gamma`: Specific heat ratio (default is 1.4).
        - `N`: Number of characteristic lines (default is 10).
        - `bell`: Boolean to determine if a bell nozzle is designed (default is True).
        - `R_c`: Radius of curvature at the throat (default is 0).
        - `theta_e`: Exit angle (default is None).
        - `plot`: Boolean to determine if the nozzle contour should be plotted (default is True).
    - `_nozzle_bell_3d(self, M_exit, D_t)`: Computes the 3D bell nozzle contour.
    - `_mach_contours(self)`: Plots Mach number contours.
...
M_exit = 2.0
D_t = 0.02
case.moc_nozzle_design(M_exit, D_t, N=N, gamma=1.4, plot=True)
...
## [Code/Implementation](https://github.com/bolanderc/Bolander-Propulsion-Demos)
```
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
