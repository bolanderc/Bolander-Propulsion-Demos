...
class incompressible_hybrid:
    ...
    def hybrid_burn_analysis(self, t_burn, M_fuel_0, dt_steady=2e-3, dt_trans=1.5e-3, total=False):
        ...
        for k in range(len(t)):
            ...
            self.Isp_accum_t[k] = self._Isp_mean(self.F_Tvac_t, t, x[2], x[3])
        ...
        thrust_fig.savefig('./Figures/Pressure_' + str_label + '.pdf')
        ...
        thrust_fig.savefig('./Figures/MassFlow_' + str_label + '.pdf')
        ...
        thrust_fig.savefig('./Figures/Regression_' + str_label + '.pdf')
        ...
        Isp_axis.legend()
        thrust_fig.savefig('./Figures/Isp_' + str_label + '.pdf')
        ...
        massrem_axis.legend()
        thrust_fig.savefig('./Figures/Mass_Remaining_' + str_label + '.pdf')
        ...
        thrust_fig.savefig('./Figures/Rdot_Gox_' + str_label + '.pdf')
        ...
        cstar_axis.legend()
        thrust_fig.savefig('./Figures/Cstar_' + str_label + '.pdf')


if __name__ == "__main__":
    plt.close('all')
    case = incompressible_hybrid(3000.0e3)
    case.hybrid_burn_analysis(6., 2.9)

    plt.close('all')
    case_total = incompressible_hybrid(3100.0e3)
    case_total.hybrid_burn_analysis(6., 3.25, total=True)
